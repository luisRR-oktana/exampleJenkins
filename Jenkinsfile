...
node {
    ...
    def SF_CONSUMER_KEY=env.ORG_CONSUMER_KEY
    def SF_USERNAME=env.ORG_USER
    def SERVER_KEY_CREDENTALS_ID=env.ORG_KEY
    def TEST_LEVEL='RunLocalTests'
    def PACKAGE_NAME='0Ho5e000000TNgeCAG'
    def PACKAGE_VERSION
    def SF_INSTANCE_URL = env.SF_INSTANCE_URL ?: "https://login.salesforce.com" //cambiar a test.salesforce.com cuando es sandbox
    ...
    withEnv(["HOME=${env.WORKSPACE}"]) {
        ...
        withCredentials([file(credentialsId: SERVER_KEY_CREDENTALS_ID, variable: 'server_key_file')]) {
           ...
           /* stage('Authorize DevHub') {
            ...
            stage('Authorize DevHub') {
                rc = command "${toolbelt}/sfdx force:auth:jwt:grant --instanceurl https://login.salesforce.com --clientid /Users/luisramirez/Documents/Jenkins/Test_Credentials/server.key --username luis.ramirez@jenkins.oktana --jwtkeyfile ${server_key_file} --setdefaultdevhubusername --setalias HubOrg"
                if (rc != 0) {
                    error 'Salesforce dev hub org authorization failed.'
                }
